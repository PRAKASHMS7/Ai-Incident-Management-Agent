...
if os.getenv("TRACING_ENABLED", "true").lower() == "true":
    try:
        ...
        logger.warning(f"Failed to initialize OTLP exporter ({e}). Falling back to ConsoleSpanExporter.")
        exporter = ConsoleSpanExporter()
else:
    logger.info("OpenTelemetry tracing disabled (TRACING_ENABLED=false). Falling back to ConsoleSpanExporter.")
    exporter = ConsoleSpanExporter()
