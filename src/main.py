...
async def lifespan(app: FastAPI):
    ...
    try:
            ...
            if topology_path.exists():
                ...
                for dep in data.get("dependencies", []):
                    neo4j_manager.create_dependency(
                        ...
                        p99_latency_threshold_ms=dep.get(
        ...
        logger.warning(
            "Neo4j setup or seeding skipped (degraded mode active): %s", str(e)
        )
...
def get_health(response: Response) -> Dict[str, Any]:
    ...
    overall_healthy = (
        redis_health["status"] == "healthy" and neo4j_health["status"] == "healthy"
    )

    result = {
        "status": "healthy" if overall_healthy else "unhealthy",
        "components": {"redis": redis_health, "neo4j": neo4j_health},
    }
