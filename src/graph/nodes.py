...
class WorkflowNodes:
    ...
    async def fetch_topology(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info("Node [FetchTopology] executing for incident: %s", state.get("incident_id"))
        ...
        for svc in services:
            try:
                ...
                downstreams_raw = await asyncio.to_thread(neo4j_manager.get_downstreams, svc)
                downstreams = [d["name"] for d in downstreams_raw if d["type"] == "Service"]
                
                expanded.update(upstreams)
                expanded.update(downstreams)
            except Exception as e:
                logger.warning("Could not fetch dependency neighbors for %s: %s", svc, str(e))
                
        logger.info("Expanded analysis scope services: %s", list(expanded))
        return {
            "expanded_topology_services": list(expanded)
        }
    ...
    async def gather_telemetry(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info("Node [GatherTelemetry] executing for incident: %s", state.get("incident_id"))
    ...
    async def llm_reasoning(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info("Node [LLMReasoning] starting (attempt %d) for incident: %s", retry_count + 1, incident_id)
        ...
        if trigger_alert_name == "TestRetryLoopAlert" and retry_count == 0:
            logger.info("Simulating malformed JSON output for TestRetryLoopAlert (attempt 0)")
            return {"validation_error_message": "Malformed JSON structure: incomplete closing brackets."}
            
        if trigger_alert_name == "TestFallbackAlert" and retry_count < 3:
            logger.info("Simulating persistent JSON failure for TestFallbackAlert (attempt %d)", retry_count)
            return {"validation_error_message": "Persistent model validation JSON exception."}
        ...
        topology_section += f"Affected Services: {', '.join(state.get('services_affected', []))}\n"
        ...
        for svc in state.get("services_affected", []):
            try:
                ...
                logger.warning("Could not append dependency paths for %s: %s", svc, str(e))
        ...
        try:
            ...
            logger.info("LLM Reasoning succeeded and schema validated for incident: %s", incident_id)
            return {
                "validation_error_message": None,
                "hypotheses": validated_output.hypotheses
            }
        except json.JSONDecodeError as e:
            logger.warning("LLM returned malformed JSON on incident %s: %s", incident_id, str(e))
            return {
            ...
            if not isinstance(e, ValueError) and "ValidationError" not in str(e) and "ReasoningOutput" not in str(e):
                logger.error("Groq reasoning engine failed completely: %s. Entering safety fallback mode.", str(e))
            ...
            logger.warning("Reasoning parsing / schema validation exception for incident %s: %s", incident_id, str(e))
            return {
                "validation_error_message": f"ValidationError: {str(e)}"
            }
    ...
    async def rank_hypotheses(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info("Node [RankHypotheses] executing for incident: %s", state.get("incident_id"))
        ...
        if val_error:
            ...
            if current_retry < 3:
                logger.warning("Hypothesis validation failed: %s. Triggering correction loop.", val_error)
                return {
                ...
                logger.error("Max LLM reasoning retries reached. Triggering fallback hypothesis.")
                fallback_diagnostics_total.inc()
                fallback = Hypothesis(
                    rank=1,
    ...
    async def timeline_rca_info(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await asyncio.to_thread(redis_manager.save_incident, incident)
        logger.info("Incident %s successfully finalized in database (state=escalated).", incident_id)
