...
def check_validation_route(state: IncidentWorkflowState) -> str:
    ...
    logger.info("Conditional Router: Format validation OK or retry cap met. Routing to [slack_escalation]")
...
def build_workflow() -> StateGraph:
    ...
    # 1. Add Nodes
    workflow.add_node("initialize_state", WorkflowNodes.initialize_state)
    workflow.add_node("fetch_topology", WorkflowNodes.fetch_topology)
    workflow.add_node("gather_telemetry", WorkflowNodes.gather_telemetry)
    workflow.add_node("llm_reasoning", WorkflowNodes.llm_reasoning)
    workflow.add_node("rank_hypotheses", WorkflowNodes.rank_hypotheses)
    workflow.add_node("slack_escalation", WorkflowNodes.slack_escalation)
    workflow.add_node("timeline_rca_info", WorkflowNodes.timeline_rca_info)
