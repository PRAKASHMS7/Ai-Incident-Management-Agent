...
from src.api.schemas import MetricPoint, MetricSeries, PrometheusTelemetryResponse
...
class PrometheusClient:
    ...
    async def get_metrics(self, service_name: str, starts_at: datetime, ends_at: datetime) -> str:
        """
        ...
        with tracer.start_as_current_span("prometheus.get_metrics") as span:
            ...
            try:
                ...
                logger.warning("Failed to query Redis cache for Prometheus telemetry: %s", str(e))
            ...
            for name, query in queries.items():
                tasks.append(self._query_range_with_semaphore(name, query, start_unix, end_unix, step))
            ...
            for name, data in results:
                if "error" in data:
                    telemetry_lines.append(f"- {name}: Telemetry query failed ({data['error']})")
                    continue
                ...
                has_data = True
                telemetry_lines.append(f"- {name}: {len(metrics_data)} timeseries returned.")
                ...
                for result_item in metrics_data:
                    ...
                    sample_vals = [f"{v.value:.4f}@{datetime.fromtimestamp(v.timestamp).strftime('%H:%M:%S')}" for v in values[-3:]]
                    telemetry_lines.append(f"  * Labels: {labels} | Recent Points: {', '.join(sample_vals)}")
            ...
            try:
                ...
                logger.warning("Failed to cache Prometheus query range results: %s", str(e))
        ...
        with tracer.start_as_current_span("prometheus.query_range") as span:
            ...
            async with prom_semaphore:
                ...
                try:
                    ...
                    return name, {"error": f"HTTP {response.status_code}: {response.text}"}
                except Exception as e:
                    logger.error("Prometheus range query failed for %s: %s", name, str(e))
