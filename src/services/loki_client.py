...
from src.api.schemas import LogLine, LogQueryResponse
...
class LokiClient:
    ...
    async def get_logs(self, service_name: str, starts_at: datetime, ends_at: datetime, limit: int = 50) -> str:
        """
        ...
        with tracer.start_as_current_span("loki.get_logs") as span:
            ...
            cache_key = f"telemetry:cache:loki:{service_name}:{rounded_start}:{rounded_end}"
            ...
            async with loki_semaphore:
                ...
                with tracer.start_as_current_span("loki.query_range") as query_span:
                    ...
                    try:
                        ...
                        query_span.set_attribute("http.status_code", response.status_code)
                        if response.status_code == 200:
                            data = response.json()
                        else:
                            data = {"error": f"HTTP {response.status_code}: {response.text}"}
                    except Exception as e:
                        logger.error("Loki query range failed for %s: %s", service_name, str(e))
        ...
        if "error" in data:
            ...
            if not log_lines:
                ...
                telemetry_lines.append(f"Retrieved {len(log_lines)} relevant error log lines:")
                ...
                if is_truncated:
                    telemetry_lines.append("[... Log trace truncated to protect LLM context windows ...]")
