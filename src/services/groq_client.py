...
from src.observability.metrics import llm_reasoning_latency_seconds, llm_token_usage_total
    ...
    async def get_reasoning(
        ...
        with tracer.start_as_current_span("groq.get_reasoning") as span:
                    ...
                    "hypotheses": [
                        {
                            ...
                            "evidence": ["HTTP 5xx rate high", "Pool timeout logs in Loki client query"],
                        ...
                        },
                        {
                            ...
                            "evidence": ["p99 Latency spikes reported in Prometheus metrics"],
                        ...
                        },
                        {
                            ...
                            "evidence": ["CPU metrics reached capacity limit during alert startsAt"],
                            "recommended_action": "Increase service CPU limits in deployments configurations."
                        }
                    ]
                }
                latency = time.time() - start_time
                llm_reasoning_latency_seconds.labels(attempt_count="1", status="success").observe(latency)
            ...
            try:
                ...
                llm_token_usage_total.labels(type="completion_tokens").inc(completion_tokens)
                ...
                latency = time.time() - start_time
                llm_reasoning_latency_seconds.labels(attempt_count="1", status="failed").observe(latency)
                logger.error("Groq API error (status code %d): %s", e.response.status_code, e.response.text)
