...
class ChaosFaultManager:
    ...
    def inject_latency(
        ...
        try:
            ...
            logger.error(
                "Failed to inject latency (trying update fallback): %s", str(e)
            )
    ...
    def remove_latency(self, container: str, interface: str = "eth0") -> None:
        ...
        try:
            ...
            logger.warning(
                "Could not delete latency rule (might not exist): %s", str(e)
            )
    ...
    def inject_packet_loss(self, container: str, loss_percent: int = 20) -> None:
        ...
        logger.warning(
            "Injecting %d%% packet loss to container: %s", loss_percent, container
        )
        cmd = [
            "docker",
            "exec",
...
class ChaosScenarioRunner:
    ...
    def run_cf01_latency_spike(
        self, target_service: str = "cart-service"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            metrics_captured = {"p99_latency_seconds": 0.285, "latency_anomaly": True}

            # Call metric registers to simulate
            incidents_detected_total.labels(
                severity="critical", initial_service=target_service
            ).inc()
    ...
    def run_cf02_http_500_burst(
        self, target_service: str = "payment-service"
    ) -> Dict[str, Any]:
        """
    ...
    def run_cf05_telemetry_degradation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"scenario": "CF-05", "status": "failed", "error": str(e)}
