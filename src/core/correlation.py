...
class CorrelationEngine:
    ...
    def correlate_alert(alert: StandardizedAlert) -> str:
        ...
        with tracer.start_as_current_span("correlate_alert") as span:
            ...
            logger.info("Correlating alert %s for service %s", alert.name, alert.service)
            ...
            for incident in active_incidents:
                ...
                time_delta = abs((alert.starts_at - incident.updated_at).total_seconds())
                if time_delta > CORRELATION_WINDOW_SECONDS:
                    continue
                    
                # B. Direct service match
                if alert.service in incident.services_affected:
                    logger.info("Direct service match found for service %s in incident %s", alert.service, incident.id)
                    matched_incidents.append(incident)
                    continue
                    
                # C. Dependency graph match (Neo4j)
                if neo4j_manager.check_dependency_path(alert.service, list(incident.services_affected)):
                    logger.info(
            ...
            if not matched_incidents:
                ...
                incidents_detected_total.labels(severity=new_incident.severity, initial_service=alert.service).inc()
                ...
                logger.info("Mapping alert %s to existing incident %s", alert.id, target_incident.id)
                ...
                logger.info("Alert %s bridges multiple incidents: %s", alert.id, [i.id for i in matched_incidents])
                primary_id = CorrelationEngine._merge_multiple_incidents(matched_incidents, alert)
    ...
    def _create_incident(alert: StandardizedAlert) -> IncidentStateModel:
        ...
        logger.info("Initializing new incident %s triggered by alert %s", incident_id, alert.id)
    ...
    @staticmethod
    def _merge_alert_into_incident(incident: IncidentStateModel, alert: StandardizedAlert) -> None:
        """
        ...
        if severity_ranking.get(alert.severity, 0) > severity_ranking.get(incident.severity, 0):
    ...
    @staticmethod
    def _merge_multiple_incidents(incidents: List[IncidentStateModel], alert: StandardizedAlert) -> str:
        """
        ...
        logger.info("Merging secondary incidents %s into primary %s", [s.id for s in secondaries], primary.id)
        ...
        severity_ranking = {"info": 1, "warning": 2, "critical": 3}
        max_severity = max([a.severity for a in primary.alerts], key=lambda s: severity_ranking.get(s, 0))
