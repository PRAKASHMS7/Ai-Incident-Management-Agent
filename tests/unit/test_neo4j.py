...
def test_get_full_graph(manager):
    ...
    mock_node_record = {
        "type": "Service",
        "props": {"name": "payment-service", "language": "python"},
    }
    mock_edge_record = {
        "source": "api-gateway",
    ...
    mock_session.run.side_effect = [[mock_node_record], [mock_edge_record]]
...
def test_check_dependency_path_cache_miss(mock_redis_mgr, manager):
    ...
    mock_record.__getitem__.side_effect = lambda key: (
        True if key == "connected" else None
    )
    ...
    mock_redis_client.set.assert_called_once_with(
        "cache:neo4j:api-gateway:payment-service", "1", ex=300
    )
