...
async def test_prom_client_cache_miss_success(mock_redis):
            ...
            "result": [
                {
                    ...
                    "values": [[1780000000.0, "0.15"], [1780000015.0, "0.18"]],
                }
            ],
        },
    }
    ...
    with patch.object(
        client.client, "get", return_value=mock_response_obj
    ) as mock_http_get:
        res = await client.get_metrics(
            service_name="payment-service",
            starts_at=datetime(2026, 5, 26, 12, 0, 0),
...
async def test_loki_client_truncation(mock_redis):
    ...
    mock_loki_response = {
        ...
        "data": {
            ...
            "result": [
                {
                    "stream": {"container": "payment-service"},
                    "values": [["1780000000000000000", long_log_line]],
                }
            ],
        },
    }
    ...
    with patch.object(client.client, "get", return_value=mock_response_obj):
        ...
        assert (
            "A" * 6000 not in res
        )  # String should have been dropped due to truncation cap
