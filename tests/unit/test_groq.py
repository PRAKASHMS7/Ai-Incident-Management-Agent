...
def test_is_retryable():
    ...
    err_429 = httpx.HTTPStatusError("Too Many Requests", request=MagicMock(), response=resp_429)
    assert is_retryable(err_429) is True

    resp_503 = httpx.Response(503, request=MagicMock())
    err_503 = httpx.HTTPStatusError("Service Unavailable", request=MagicMock(), response=resp_503)
...
def test_reasoning_output_validation():
    ...
    valid_data = {
        "hypotheses": [
            {"rank": 1, "hypothesis": "H1", "confidence_score": 0.9, "evidence": ["E1"], "recommended_action": "A1"},
            {"rank": 2, "hypothesis": "H2", "confidence_score": 0.7, "evidence": ["E2"], "recommended_action": "A2"},
            {"rank": 3, "hypothesis": "H3", "confidence_score": 0.5, "evidence": ["E3"], "recommended_action": "A3"}
        ]
    }
    ...
    invalid_data_4 = {
        "hypotheses": [
            ...
            {"rank": 2, "hypothesis": "H2", "confidence_score": 0.7, "evidence": ["E2"], "recommended_action": "A2"},
            {"rank": 3, "hypothesis": "H3", "confidence_score": 0.5, "evidence": ["E3"], "recommended_action": "A3"},
            {"rank": 4, "hypothesis": "H4", "confidence_score": 0.3, "evidence": ["E4"], "recommended_action": "A4"}
        ]
    }
    with pytest.raises(ValidationError):
...
async def test_groq_client_retry_logic():
    ...
    with patch.object(client.client, "post", side_effect=[mock_response_429, mock_response_429, mock_response_200]) as mock_post:
