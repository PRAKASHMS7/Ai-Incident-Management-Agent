...
def test_metrics_incrementation():
    ...
    incidents_detected_total.labels(
        severity="critical", initial_service="test-service"
    ).inc()
    ...
    # 4. LLM latency
    llm_reasoning_latency_seconds.labels(attempt_count="1", status="success").observe(
        0.5
    )
    ...
    slack_escalation_delivery_total.labels(
        channel="test-channel", status="success"
    ).inc()
    ...
    assert (
        'incidents_detected_total{initial_service="test-service",severity="critical"} 1.0'
        in response.text
    )
    assert (
        'alert_deduplicated_total{alertname="TestAlert",service="test-service"} 1.0'
        in response.text
    )
