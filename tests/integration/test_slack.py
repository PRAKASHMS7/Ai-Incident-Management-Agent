...
from src.api.schemas import IncidentStateModel, StandardizedAlert
...
def create_incident(mock_redis_db):
    ...
    def _create(
        incident_id: str, state: str = "open", severity: str = "critical"
    ) -> IncidentStateModel:
...
async def test_slack_interactive_acknowledge_endpoint(mock_redis_db, create_incident):
    ...
    payload = {
        "type": "block_actions",
        "user": {"id": "U12345", "name": "alice", "team_id": "T12345"},
        "team": {"id": "T12345", "domain": "workspace-domain"},
        "container": {
            "type": "message",
    ...
    mock_auth_test = MockSlackResponse(
        {"ok": True, "bot_id": "B12345", "user_id": "U12345", "team_id": "T12345"}
    )

    # Post to FastAPI /slack/events path with mocked signature verification and auth_test
    with patch(
        "slack_sdk.signature.SignatureVerifier.is_valid", return_value=True
    ), patch(
        ...
        async with httpx.AsyncClient(
            ...
            assert any(
                item.event_type == "operator_action" for item in updated.timeline
            )
...
async def test_slack_post_escalation_failure_fallback(mock_redis_db, create_incident):
    ...
    mock_err = slack_sdk.errors.SlackApiError(
        message="Fatal API Outage", response={"ok": False, "error": "fatal_outage"}
    )
    ...
    with patch(
        ...
        with patch("tenacity.nap.time.sleep", return_value=None):
            ...
            assert updated is not None
            assert any(
                item.event_type == "escalation_failed" for item in updated.timeline
            )
