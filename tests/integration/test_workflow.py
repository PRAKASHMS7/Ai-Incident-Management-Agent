...
def mock_redis_db():
    ...
    def mock_get_active():
        return [i for i in saved_incidents.values() if I.state in ["open", "analyzing", "awaiting_approval"]]
    ...
    with patch("src.database.redis_client.RedisClientManager.get_client") as mock_get_client, \
         patch("src.database.redis_client.RedisClientManager.save_incident", side_effect=mock_save), \
         patch("src.database.redis_client.RedisClientManager.get_incident", side_effect=mock_get), \
         patch("src.database.redis_client.RedisClientManager.get_active_incidents", side_effect=mock_get_active):
...
def mock_neo4j():
    ...
    with patch("src.database.neo4j_client.Neo4jClientManager.get_upstreams") as mock_up, \
         patch("src.database.neo4j_client.Neo4jClientManager.get_downstreams") as mock_down:
...
def mock_telemetry():
    ...
    with patch("src.services.prom_client.PrometheusClient.get_metrics") as mock_prom, \
         patch("src.services.loki_client.LokiClient.get_logs") as mock_loki:
...
def create_incident():
    ...
    def _create(incident_id: str, alert_name: str = "Http5xxRateHigh") -> IncidentStateModel:
...
@pytest.mark.asyncio
async def test_workflow_e2e_success_path(mock_redis_db, mock_neo4j, mock_telemetry, create_incident):
    """
    ...
    """
    inc = create_incident("inc-e2e-ok")
    
    config = {"configurable": {"thread_id": "inc-e2e-ok"}}
    result = await compiled_workflow.ainvoke(
        {"incident_id": "inc-e2e-ok"},
        config=config
    )
...
@pytest.mark.asyncio
async def test_workflow_retry_path(mock_redis_db, mock_neo4j, mock_telemetry, create_incident):
    """
    Verifies that formatting errors trigger the retry loop in the workflow.
    """
    inc = create_incident("inc-retry", alert_name="TestRetryLoopAlert")
...
@pytest.mark.asyncio
async def test_workflow_fallback_path(mock_redis_db, mock_neo4j, mock_telemetry, create_incident):
    """
    Verifies that if formatting errors persist, the workflow triggers the safety fallback
    path after max retries are exceeded.
    """
    inc = create_incident("inc-fallback", alert_name="TestFallbackAlert")
...
@pytest.mark.asyncio
async def test_workflow_checkpoint_recovery_and_resume(mock_redis_db, mock_neo4j, mock_telemetry, create_incident):
    """
    ...
    """
    inc = create_incident("inc-crash-recover", alert_name="TestCrashAlert")
    ...
    with pytest.raises(ValueError, match="Database error!"):
        await compiled_workflow.ainvoke(
            {"incident_id": "inc-crash-recover"},
            config=config
        )
        
    # Check that Redis created checkpoint records up to the crash point
    checkpoint_keys = [k for k in mock_redis_db.set.call_args_list if k[0][0].startswith("checkpoint:inc-crash-recover")]
    ...
    resume_result = await compiled_workflow.ainvoke(
        None,
        config=config
    )
