...
from src.api.schemas import IncidentStateModel, StandardizedAlert, TimelineItem, Hypothesis
...
def mock_redis_db():
    ...
    with patch("src.database.redis_client.RedisClientManager.save_incident", side_effect=mock_save), \
         ...
         patch("src.database.redis_client.RedisClientManager.get_client") as mock_get_client:
...
def create_incident(mock_redis_db):
    ...
    def _create(incident_id: str, state: str = "open") -> IncidentStateModel:
        ...
        incident = IncidentStateModel(
            ...
            alerts=[alert],
            timeline=[
                TimelineItem(timestamp=alert.starts_at, event_type="alert_triggered", source="prometheus", message="Fired", severity="critical")
            ],
            hypotheses=[
                Hypothesis(rank=1, hypothesis="Hyp1", confidence_score=0.8, evidence=["E1"], recommended_action="A1")
            ],
...
def test_resolve_incident_groq_fallback(mock_redis_db, create_incident, client):
    ...
    with patch("src.services.rca_generator.settings.GROQ_API_KEY", "xoxb-groq-key"), \
         patch("src.services.groq_client.groq_client.get_reasoning", side_effect=Exception("Groq API Outage")):
